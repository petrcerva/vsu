...
def drawSigmoid(*args, **kwargs):
    kwargs.pop("softmax", None)
    drawSoftmax(*args, **kwargs)

def drawSigmoidShape(sigmoid, title=None):
    x = np.linspace(-10, 10, 150)
    y = sigmoid(x)

    fig, ax = plt.subplots(figsize=(10, 5))

    sc = ax.scatter(x, y, c=y, cmap="RdBu_r", s=15)

    if title is not None:
        ax.set_title(title)
    ax.set_xlabel("výstup lineární vrstvy")
    ax.set_ylabel("výstup sigmoidy")
    ax.grid(True)

    plt.show()
    
def drawSoftmax(data, classes, theta=None, sigmoid=None, softmax=None, title='Data', x_label='x1', y_label='x2'):
    ...
    if(theta is not None and softmax is None and sigmoid is None):
        ax = plt.gca()
        u = np.linspace(np.floor(np.min(data)), np.floor(np.max(data)) + 1, 10)
        v = np.linspace(np.floor(np.min(data)), np.floor(np.max(data)) + 1, 10)
        z = np.zeros([10, 10]);
    ...
    elif (theta is not None and softmax is not None):
        #krivky
        ax = plt.gca()
    ...
    elif (theta is not None and sigmoid is not None):
        # Sigmoid decision boundary for binary classification
        ax = plt.gca()
        u = np.linspace(np.floor(np.min(data)), np.floor(np.max(data)) + 1, 250)
        v = np.linspace(np.floor(np.min(data)), np.floor(np.max(data)) + 1, 250)
        z = np.zeros([250, 250])
        ...
        for i in range(250):
            for j in range(250):
                z[i,j] = sigmoid(np.dot([1, u[i], v[j]], theta))

        CS = ax.contourf(u, v, z.T, levels=20, cmap=plt.cm.RdBu_r, alpha=0.6)
        ax.clabel(CS, inline=True, fontsize=10)
        plt.colorbar(CS, ax=ax, label="Predikovaná pravděpodobnost žluté třídy")
